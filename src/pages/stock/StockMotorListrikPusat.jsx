...
const LS_LOCAL = "MMT_STOCK_MOLIS_LOCAL_V1";
const LS_HIDDEN = "MMT_STOCK_MOLIS_HIDDEN_V1";
...
function molisKey(row) {
  return (row.noDinamo || row.namaBarang || "").toString().trim().toLowerCase();
}

function normalizeBaseMolis(raw, tokoName) {
  const nama = raw.namaBarang || raw.nama || raw.name || raw.product || raw.tipe || "";
  const noDinamo = (raw.noDinamo || raw.no_dinamo || raw.imei || raw.serial || "").toString();
  ...
  return {
    id: `${tokoName}::base::${molisKey({ noDinamo, namaBarang: nama })}`,
    source: "base",
...
function normalizeLocalMolis(raw, tokoName) {
  return {
    ...
    noDinamo: (raw.noDinamo || "").toString(),
...
export default function StockMotorListrikPusat({ user }) {
  ...
  const baseRows = useMemo(() => {
    ...
    for (const id of selectedTokoIds) {
      ...
      const molis = idx?.motor_listrik || [];
      for (const r of molis) out.push(normalizeBaseMolis(r, tokoName));
    }
    return out;
  }, [selectedTokoIds]);
  ...
  const mergedRows = useMemo(() => {
    ...
    for (const r of baseRows) {
      const key = `${r.tokoName}::${molisKey(r)}`;
      if (!hiddenSet.has(key)) byKey.set(key, r);
    }
    for (const id of selectedTokoIds) {
      const tokoName = TOKO_LABELS[id];
      const locals = (localMap[tokoName] || []).map((x) => normalizeLocalMolis(x, tokoName));
      for (const lr of locals) {
        const key = `${lr.tokoName}::${molisKey(lr)}`;
        if (molisKey(lr)) byKey.set(key, { ...lr, source: "local" });
  ...
  const addRow = () => {
    ...
    const next = {
      ...
      noDinamo: (form.noDinamo || "").toString(),
  ...
  const deleteRow = (row) => {
    ...
    if (row.source === "local") {
      const map = loadLocal();
      const arr = Array.isArray(map[tokoName]) ? [...map[tokoName]] : [];
      const next = arr.filter((x) => normalizeLocalMolis(x, tokoName).id !== row.id);
      map[tokoName] = next;
      saveLocal(map);
      setLocalMap(map);
    } else {
      const key = `${row.tokoName}::${molisKey(row)}`;
  ...
  const onImport = async (e) => {
    ...
    try {
      ...
      const rowsToAdd = json.map((r) => {
        ...
        return normalizeLocalMolis(
          {
            tanggal: String(pick("tanggal", "tgl", "date")).slice(0, 10) || TODAY,
            namaBarang: pick("nama barang", "nama", "product", "model", "tipe"),
            noDinamo: pick("no dinamo", "dinamo", "imei", "serial"),
  ...
  const onExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Stock MOLIS");
    const label =
      scope === "all" ? "SEMUA_TOKO" : TOKO_LABELS[Number(scope)].replace(/\s+/g, "_");
    const ymd = new Date().toISOString().slice(0, 10).replace(/-/g, "");
    XLSX.writeFile(wb, `STOCK_MOTOR_LISTRIK_${label}_${ymd}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Produk & Stock — Motor Listrik</h1>
      ...
      <p className="text-xs text-slate-500">
        Data dasar dari <code>getStockIndex(tokoName).motor_listrik</code>. Tambahan/override/hidden
