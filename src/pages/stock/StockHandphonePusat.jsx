...
/* ================= LocalStorage per kategori ================= */
const LS_LOCAL = "MMT_STOCK_HP_LOCAL_V1";     // tambahan & override per toko
const LS_HIDDEN = "MMT_STOCK_HP_HIDDEN_V1";   // daftar key yang disembunyikan (base)
...
/* ================= Normalizer base row ================= */
function hpKey(row) {
  const k = (row.imei || row.namaBarang || "").toString().trim().toLowerCase();
  return k;
}
function normalizeBaseHp(raw, tokoName) {
  const nama =
    raw.namaBarang || raw.nama || raw.name || raw.product || raw.model || raw.tipe || "";
  const imei = (raw.imei || raw.serial || "").toString();
  ...
  return {
    id: `${tokoName}::base::${hpKey({ imei, namaBarang: nama })}`,
    source: "base",
    tanggal: parseDateInput(raw.tanggal) || "",
    tokoName,
    namaBarang: nama,
    imei,
    stokSistem,
...
function normalizeLocalHp(raw, tokoName) {
  return {
    id: raw.id || `${tokoName}::local::${Date.now()}::${Math.random().toString(36).slice(2, 8)}`,
    source: raw.source || "local",
    tanggal: parseDateInput(raw.tanggal) || TODAY,
    tokoName,
...
/* ================= Page ================= */
export default function StockHandphonePusat({ user }) {
  ...
  // ===== Data dasar dari file (gabungan per toko terpilih) =====
  const baseRows = useMemo(() => {
    const list = [];
    for (const id of selectedTokoIds) {
      const tokoName = TOKO_LABELS[id];
      const idx = getStockIndex(tokoName);
      const hp = idx?.handphone || [];
      for (const r of hp) list.push(normalizeBaseHp(r, tokoName));
    }
    return list;
  }, [selectedTokoIds]);

  // ===== LS tambahan/override & hidden per toko =====
  const [localMap, setLocalMap] = useState(() => loadLocal());
  const [hiddenMap, setHiddenMap] = useState(() => loadHidden());

  // gabungkan base + local (dengan overlay & hidden)
  const mergedRows = useMemo(() => {
    ...
    // Buat map dari base by key
    const byKey = new Map();
    for (const r of baseRows) {
      const key = `${r.tokoName}::${hpKey(r)}`;
      if (!hiddenSet.has(key)) {
        byKey.set(key, r);
      }
    }

    // overlay + tambahan local
    for (const id of selectedTokoIds) {
      const tokoName = TOKO_LABELS[id];
      const locals = (localMap[tokoName] || []).map((x) => normalizeLocalHp(x, tokoName));
      for (const lr of locals) {
        const key = `${lr.tokoName}::${hpKey(lr)}`;
        if (hpKey(lr)) {
          // override baris base
          byKey.set(key, { ...lr, source: "local" });
        } else {
          // tambahan tanpa key unik (tanpa imei/nama) → tetap push
          byKey.set(lr.id, lr);
        }
      }
    }
  ...
  // sinkron toko form saat scope berganti
  useEffect(() => {
    setForm((f) => ({
      ...f,
      tokoId: (selectedTokoIds[0] ?? ALL_TOKO_IDS[0]),
  ...
  const saveEdit = () => {
    ...
    const key = hpKey(draft);
    const map = loadLocal();
    const arr = Array.isArray(map[tokoName]) ? [...map[tokoName]] : [];

    // Jika draft berasal dari base → kita override (replace by key)
    if (draft.source === "base") {
      ...
      // hapus local lama dengan key sama (jika ada)
      const filtered = arr.filter((x) => hpKey(x) !== key);
      map[tokoName] = [...filtered, override];
    } else {
      // edit pada local → replace by id
      map[tokoName] = arr.map((x) =>
        (normalizeLocalHp(x, tokoName).id === draft.id) ? {
          source: "local",
          tanggal: draft.tanggal || TODAY,
          namaBarang: draft.namaBarang || "",
          imei: (draft.imei || "").toString(),
  ...
  const deleteRow = (row) => {
    ...
    if (row.source === "local") {
      ...
      const map = loadLocal();
      const arr = Array.isArray(map[tokoName]) ? [...map[tokoName]] : [];
      const next = arr.filter((x) => normalizeLocalHp(x, tokoName).id !== row.id);
      map[tokoName] = next;
      saveLocal(map);
      setLocalMap(map);
    } else {
      // base: tandai hidden
      const key = `${row.tokoName}::${hpKey(row)}`;
  ...
  const onImport = async (e) => {
    ...
    try {
      ...
      const rowsToAdd = json.map((r) => {
        ...
        return normalizeLocalHp(
          {
            tanggal: parseXlsxDate(pick("tanggal", "tgl", "date")),
  ...
  const onExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Stock HP");
    const label =
      scope === "all" ? "SEMUA_TOKO" : TOKO_LABELS[Number(scope)].replace(/\s+/g, "_");
    const ymd = new Date().toISOString().slice(0, 10).replace(/-/g, "");
    XLSX.writeFile(wb, `STOCK_HANDPHONE_${label}_${ymd}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Produk & Stock — Handphone</h1>
          <p className="text-slate-600">
            Pemeriksaan stock pusat {canPickAll ? "(semua toko)" : "(toko sendiri)"}.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <label
            ...
            title="Import akan menambah data lokal (per toko)"
          >
            Import Excel
          </label>
          <button
                  ...
                  <tr key={r.id} className="border-b last:border-0 bg-slate-50/60">
                    ...
                    <td className="px-3 py-2">
                      <input
                        className="border rounded px-2 py-1"
                        value={draft.imei}
                        onChange={(e) => setDraft((d) => ({ ...d, imei: e.target.value }))}
                      />
                    </td>
      ...
      <p className="text-xs text-slate-500">
        Data dasar dari <code>getStockIndex(tokoName).handphone</code>. Tambahan/override/hidden
        disimpan per toko di <code>localStorage</code>:
        <code> {LS_LOCAL}</code> & <code>{LS_HIDDEN}</code>.
      </p>
    </div>
  );
}
