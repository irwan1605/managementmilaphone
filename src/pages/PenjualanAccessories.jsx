// src/pages/PenjualanAccessories.jsx
import React, { useEffect, useMemo, useState } from "react";
import * as XLSX from "xlsx";

import { getBrandIndex, getProductsByBrand, findHarga } from "../data/MasterDataHargaPenjualan";
import {
  TOKO_LIST,
  PAYMENT_METHODS,
  PRICE_CATEGORIES,
...
/* Ambil daftar warna dari brand index (kalau tidak ada ya kosong) */
...
function computeFinancials(row) {
  ...
  if (method === "kredit") {
    ...
    return {
      base, addOns, subtotal, mdrPct, mdrFee, net,
      dp, tenor, bungaRate, principal, totalBunga, cicilan, grandTotal,
    };
  }
  return {
    base, addOns, subtotal, mdrPct, mdrFee, net,
    dp: toNum(row.dp), tenor: 0, bungaRate: 0, principal: subtotal, totalBunga: 0, cicilan: 0, grandTotal: subtotal,
  };
}

/* ========== Komponen ========== */
export default function PenjualanAccessories() {
  /* Data master */
  const brandIndex = useMemo(() => getBrandIndex() || [], []);
  const brandOptions = useMemo(() => Array.from(new Set((brandIndex || []).map((b) => b.brand))), [brandIndex]);

  /* Storage */
  const LS_KEY = "accessories_sales_pusat_v1";
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (ql) {
        const hay = [
          r.produk, r.brand, r.serial, r.salesName, r.leasingName, r.tokoRef, r.storeName, r.note,
        ].join(" ").toLowerCase();
  ...
  const [form, setForm] = useState({
    ...
    produk: "",
    warna: "",
    serial: "", // IMEI / Serial acc (opsional)
    qty: 1,
    hargaType: "SRP",
    srp: 0,
    grosir: 0,
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ACC_PENJUALAN");
    const ymd = new Date().toISOString().slice(0, 10).replace(/-/g, "");
    XLSX.writeFile(wb, `PENJUALAN_ACCESSORIES_PUSAT_${ymd}.xlsx`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => window.history.back()} className="rounded-lg border bg-white px-3 py-2 text-sm shadow-sm hover:bg-slate-50">← Kembali</button>
          <button onClick={exportExcel} className="rounded-lg border bg-white px-3 py-2 text-sm shadow-sm hover:bg-slate-50">Export (.xlsx)</button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          <div className="md:col-span-3">
            ...
            <input className="w-full border rounded px-2 py-1" placeholder="produk/serial/sales/leasing/note" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
          <div className="md:col-span-3">
            <label className="text-xs text-slate-600">Toko</label>
            <select className="w-full border rounded px-2 py-1" value={filterToko} onChange={(e) => setFilterToko(e.target.value)}>
              <option>Semua</option>
          ...
          <div className="md:col-span-2">
            ...
            <select className="w-full border rounded px-2 py-1" value={filterMethod} onChange={(e) => setFilterMethod(e.target.value)}>
              <option>Semua</option>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="text-sm text-slate-500">Transaksi</div><div className="mt-1 text-2xl font-semibold">{summary.count}</div></div>
        <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="text-sm text-slate-500">Total Qty</div><div className="mt-1 text-2xl font-semibold">{summary.qty}</div></div>
        <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="text-sm text-slate-500">Subtotal</div><div className="mt-1 text-2xl font-semibold">{fmtIDR(summary.subtotal)}</div></div>
        <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="text-sm text-slate-500">NET</div><div className="mt-1 text-2xl font-semibold">{fmtIDR(summary.net)}</div></div>
      </div>
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          <div>
            ...
            <input type="date" className="w-full border rounded px-2 py-1" value={form.tanggal} onChange={(e) => setForm({ ...form, tanggal: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-slate-600">Toko</label>
            <select className="w-full border rounded px-2 py-1" value={form.tokoRef} onChange={(e) => setForm({ ...form, tokoRef: e.target.value, salesName: "", storeName: "", shName: "", slName: "" })}>
              {TOKO_LIST.map((t) => (<option key={t} value={t}>{t}</option>))}
            </select>
          </div>
          <div className="md:col-span-2">
            <label className="text-xs text-slate-600">Sales</label>
            <select className="w-full border rounded px-2 py-1" value={form.salesName} onChange={(e) => setForm({ ...form, salesName: e.target.value })}>
              <option value="">— Pilih Sales —</option>
              {(getSalesByToko(form.tokoRef) || []).map((s) => (<option key={s.nik || s.name} value={s.name}>{s.name}</option>))}
            </select>
          </div>
          <div>
            <label className="text-xs text-slate-600">Store</label>
            <input className="w-full border rounded px-2 py-1" value={form.storeName} onChange={(e) => setForm({ ...form, storeName: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-slate-600">NIK</label>
            <input className="w-full border rounded px-2 py-1" value={form.nik} onChange={(e) => setForm({ ...form, nik: e.target.value })} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mt-3">
          <div>
            ...
            <select className="w-full border rounded px-2 py-1" value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value, produk: "", warna: "" })}>
          ...
          <div className="md:col-span-2">
            ...
            <select className="w-full border rounded px-2 py-1" value={form.produk} onChange={(e) => setForm({ ...form, produk: e.target.value, warna: "" })}>
              <option value="">— Pilih Produk —</option>
              {productOptions.map((p) => (<option key={`${form.brand}-${p}`} value={p}>{p}</option>))}
            </select>
          </div>
          <div>
          ...
          <div>
            <label className="text-xs text-slate-600">Serial/IMEI</label>
            <input className="w-full border rounded px-2 py-1" value={form.serial} onChange={(e) => setForm({ ...form, serial: e.target.value })} />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mt-3">
          ...
          <div>
            ...
            <input readOnly className="w-full border rounded px-2 py-1 bg-slate-50 text-right" value={fmtIDR(finPreview.subtotal)} />
          </div>
          <div>
            <label className="text-xs text-slate-600">MDR % / Fee</label>
            <div className="grid grid-cols-2 gap-2">
              <input readOnly className="border rounded px-2 py-1 bg-slate-50 text-right" value={mdrAutoPct.toFixed(2)} />
              <input readOnly className="border rounded px-2 py-1 bg-slate-50 text-right" value={`- ${fmtIDR(finPreview.mdrFee)}`} />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mt-3">
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className="border rounded px-2 py-1" value={form.paymentMethod} onChange={(e) => setForm({ ...form, paymentMethod: e.target.value })}>
                {PAYMENT_METHODS.map((m) => (<option key={m} value={m}>{m}</option>))}
              </select>
              <input className="border rounded px-2 py-1 bg-slate-50 text-right" readOnly value={fmtIDR(finPreview.net)} />
            </div>
          </div>

          <div>
            <label className="text-xs text-slate-600">Leasing/Bank</label>
            <input className="w-full border rounded px-2 py-1" placeholder="Nama Leasing/Bank" value={form.leasingName} onChange={(e) => setForm({ ...form, leasingName: e.target.value })} />
          </div>

          <div>
            <label className="text-xs text-slate-600">Tenor</label>
            <select className="w-full border rounded px-2 py-1" value={form.tenor} onChange={(e) => setForm({ ...form, tenor: toNum(e.target.value) })} disabled={String(form.paymentMethod).toLowerCase() !== "kredit"}>
          ...
          <div>
            ...
            <input readOnly className="w-full border rounded px-2 py-1 bg-slate-50 text-right" value={Number(form.bunga || 0).toFixed(2)} />
          </div>

          <div>
            <label className="text-xs text-slate-600">Cicilan/Bln</label>
            <input readOnly className="w-full border rounded px-2 py-1 bg-slate-50 text-right" value={fmtIDR(finPreview.cicilan)} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mt-3">
          <div>
            ...
            <input type="number" min={0} className="w-full border rounded px-2 py-1 text-right" value={form.dp} onChange={(e) => setForm({ ...form, dp: toNum(e.target.value) })} />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <input className="w-full border rounded px-2 py-1" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} />
          </div>
        </div>
            ...
            <tbody>
              {filteredRows.map((row) => {
                ...
                if (isEditing && editDraft) {
                  ...
                  const productOpts = (editDraft.brand && getProductsByBrand(editDraft.brand)) || [];
                  const warnaOpts = getWarnaOptions(editDraft.brand, editDraft.produk);
                  const mdrPctRow = Number(getMdr({ method: editDraft.paymentMethod, toko: editDraft.tokoRef, brand: editDraft.brand }) || 0);

                  return (
                    <tr key={row.id} className="border-b last:border-0 bg-slate-50/40">
                      <td className="px-3 py-2"><input type="date" className="border rounded px-2 py-1" value={editDraft.tanggal} onChange={(e) => setEditDraft((d) => ({ ...d, tanggal: e.target.value }))} /></td>
                      <td className="px-3 py-2">
                        <select className="border rounded px-2 py-1" value={editDraft.tokoRef} onChange={(e) => setEditDraft((d) => ({ ...d, tokoRef: e.target.value, salesName: "", storeName: "", shName: "", slName: "" }))}>
                          {TOKO_LIST.map((t) => (<option key={t} value={t}>{t}</option>))}
                        </select>
                      </td>
                      <td className="px-3 py-2"><input className="border rounded px-2 py-1" value={editDraft.storeName || ""} onChange={(e) => setEditDraft((d) => ({ ...d, storeName: e.target.value }))} /></td>
                      <td className="px-3 py-2">
                        <select className="border rounded px-2 py-1" value={editDraft.salesName || ""} onChange={(e) => setEditDraft((d) => ({ ...d, salesName: e.target.value }))}>
                          <option value="">— Pilih —</option>
                          {(getSalesByToko(editDraft.tokoRef || "") || []).map((s) => (<option key={s.nik || s.name} value={s.name}>{s.name}</option>))}
                        </select>
                      </td>
                      <td className="px-3 py-2">
                        <select className="border rounded px-2 py-1" value={editDraft.brand || ""} onChange={(e) => setEditDraft((d) => ({ ...d, brand: e.target.value, produk: "", warna: "" }))}>
                          <option value="">— Brand —</option>
                          {brandOpts.map((b) => (<option key={b} value={b}>{b}</option>))}
                        </select>
                      </td>
                      <td className="px-3 py-2">
                        <select className="border rounded px-2 py-1" value={editDraft.produk || ""} onChange={(e) => setEditDraft((d) => ({ ...d, produk: e.target.value, warna: "" }))}>
                          <option value="">— Produk —</option>
                          {productOpts.map((p) => (<option key={`${editDraft.brand}-${p}`} value={p}>{p}</option>))}
                        </select>
                      </td>
                      <td className="px-3 py-2">
                        <select className="border rounded px-2 py-1" value={editDraft.warna || ""} onChange={(e) => setEditDraft((d) => ({ ...d, warna: e.target.value }))}>
                          <option value="">— Warna —</option>
                          {warnaOpts.map((w) => (<option key={w} value={w}>{w}</option>))}
                        </select>
                      </td>
                      <td className="px-3 py-2"><input className="border rounded px-2 py-1" value={editDraft.serial || ""} onChange={(e) => setEditDraft((d) => ({ ...d, serial: e.target.value }))} /></td>
                      <td className="px-3 py-2 text-right"><input type="number" className="border rounded px-2 py-1 text-right w-24" value={editDraft.qty} onChange={(e) => setEditDraft((d) => ({ ...d, qty: toNum(e.target.value) }))} /></td>
                      <td className="px-3 py-2 text-right"><input type="number" className="border rounded px-2 py-1 text-right w-28" value={editDraft.harga} onChange={(e) => setEditDraft((d) => ({ ...d, harga: toNum(e.target.value) }))} /></td>
                      ...
                      <td className="px-3 py-2">
                        <select className="border rounded px-2 py-1" value={editDraft.paymentMethod} onChange={(e) => setEditDraft((d) => ({ ...d, paymentMethod: e.target.value, tenor: 0, bunga: 0 }))}>
                          {PAYMENT_METHODS.map((m) => (<option key={m} value={m}>{m}</option>))}
                        </select>
                      </td>
                      <td className="px-3 py-2"><input className="border rounded px-2 py-1" value={editDraft.leasingName || ""} onChange={(e) => setEditDraft((d) => ({ ...d, leasingName: e.target.value }))} /></td>
                      <td className="px-3 py-2 text-right">
                        <select className="border rounded px-2 py-1 w-24" value={editDraft.tenor || 0} onChange={(e) => {
                          const t = toNum(e.target.value);
                          const bungaAuto = getBungaByTenor({ tenor: t, method: editDraft.paymentMethod, brand: editDraft.brand, toko: editDraft.tokoRef });
                      ...
                      <td className="px-3 py-2 text-right"><input readOnly className="border rounded px-2 py-1 bg-slate-50 text-right w-20" value={Number(editDraft.bunga || 0).toFixed(2)} /></td>
                      <td className="px-3 py-2 text-right">{fmtIDR(f.cicilan)}</td>
                      <td className="px-3 py-2 text-right">{fmtIDR(f.grandTotal)}</td>
                      <td className="px-3 py-2"><input className="border rounded px-2 py-1 w-44" value={editDraft.note || ""} onChange={(e) => setEditDraft((d) => ({ ...d, note: e.target.value }))} /></td>
                      <td className="px-3 py-2">
                        <div className="flex gap-2">
                          <button onClick={saveEdit} className="px-2 py-1 text-xs rounded bg-green-600 text-white hover:bg-green-700">Simpan</button>
                          <button onClick={cancelEdit} className="px-2 py-1 text-xs rounded bg-slate-100 hover:bg-slate-200">Batal</button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                const mdrPctStatic = Number(getMdr({ method: row.paymentMethod, toko: row.tokoRef, brand: row.brand }) || 0);
                ...
                return (
                  <tr key={row.id} className="border-b last:border-0">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex gap-2">
                        <button onClick={() => beginEdit(row)} className="px-2 py-1 text-xs rounded bg-blue-600 text-white hover:bg-blue-700">Edit</button>
                        <button onClick={() => deleteRow(row.id)} className="px-2 py-1 text-xs rounded bg-red-600 text-white hover:bg-red-700">Hapus</button>
                      </div>
                    </td>
                  </tr>
                );
              ...
              {filteredRows.length === 0 && (
                <tr>
                  <td colSpan={21} className="px-3 py-6 text-center text-slate-500">Belum ada data sesuai filter/kata kunci.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <p className="text-xs text-slate-500">
        Bunga otomatis via <code>getBungaByTenor</code>, MDR otomatis via <code>getMdr</code>, warna dari <code>getBrandIndex()</code>, harga via <code>findHarga</code>.
      </p>
    </div>
  );
}
