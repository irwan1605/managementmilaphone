// src/pages/ServiceHandphone.jsx
...
/* ================= LocalStorage ================= */
const LS_KEY = "MMT_SERVICE_HP_V1";
...
/* ================= Page ================= */
export default function ServiceHandphone({ user }) {
  ...
  const picTokoName = picTokoId ? TOKO_LABELS[picTokoId] : null;

  // Sumber stok untuk saran (diambil dari toko yang sedang dipilih sebagai bengkel penerima)
  ...
  const stockIdx = useMemo(() => getStockIndex(serviceTokoName), [serviceTokoName]);
  const hpIndex = stockIdx?.handphone || [];

  const brandOptions = useMemo(
    () => unique(hpIndex.map((x) => (x.brand || x.merek || "").toString().trim()).filter(Boolean)),
    [hpIndex]
  );
  const productOptions = useMemo(() => {
    if (!brandOptions.length) return unique(hpIndex.map((x) => x.nama || x.product || x.model));
    return unique(
      hpIndex
        .filter(
          (x) => !form.brand || (x.brand || x.merek) === form.brand
        )
        .map((x) => x.nama || x.product || x.model)
    );
    // eslint-disable-next-line
  }, [hpIndex, brandOptions]); // form referenced below, but we suppress exhaustive-deps—safe here.

  const warnaOptions = useMemo(() => {
    return unique(hpIndex.map((x) => x.warna).filter(Boolean));
  }, [hpIndex]);

  // teknisi list dari sales data toko penerima
  ...
  // filter untuk PIC: hanya lihat servicenya di tokonya
  ...
  const [form, setForm] = useState(() => ({
    ...
    biayaSparepart: 0,
    ongkosService: 0,
    status: "Masuk", // Masuk | Proses | Selesai | Diambil | Cancel
    teknisi: "",
    estimasiSelesai: "",
    tanggalSelesai: "",
    catatan: "",
  ...
  // sinkron pilihan tokoService di header dengan form.tokoServiceId
  useEffect(() => {
  ...
  const onExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Service HP");
    const ymd = new Date().toISOString().slice(0, 10).replace(/-/g, "");
    XLSX.writeFile(wb, `SERVICE_HANDPHONE_${TOKO_LABELS[serviceTokoId]}_${ymd}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Service Handphone</h1>
      ...
      {/* Filter/Pemilih Toko Service (non-PIC) */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mt-3">
          <div className="md:col-span-2">
            ...
            <select
              className="w-full border rounded px-2 py-1"
              value={form.brand}
              onChange={(e) => setForm({ ...form, brand: e.target.value, produk: "" })}
            >
          ...
          <div className="md:col-span-2">
            <label className="text-xs text-slate-600">Model/Produk</label>
            <select
              className="w-full border rounded px-2 py-1"
              value={form.produk}
      ...
      <p className="text-xs text-slate-500">
        ...
        handphone diambil dari stok toko aktif via <code>getStockIndex(tokoName)</code>.
      </p>
    </div>
  );
}
