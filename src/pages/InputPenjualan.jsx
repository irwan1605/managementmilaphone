...
function normalizeRowFromExcel(row) {
  ...
  const tanggal = parseXlsxDate(pick("tanggal", "tgl", "tgl transaksi", "tanggal transaksi", "date"));
  ...
  const produk = pick("produk", "product", "type", "tipe", "sepeda listrik", "nama") || "";
  ...
  const hargaTypeRaw = (pick("harga dipakai", "tipe harga", "price type") || "").toString();
  ...
  const harga = toNum(pick("harga", "price", "payment user (dashboard)")) || (grosir || srp);
  ...
  const dpMerchant = toNum(pick("dp user via merchant (piutang)", "dp merchant (piutang)", "dp merchant"));
  const dpToko = toNum(pick("dp user ke toko (cash)", "dp toko (cash)", "dp toko"));
  ...
  const tglPengambilan = parseXlsxDate(pick("tgl pengambilan unit", "tgl pegambilan unit"));
...
export default function InputPenjualan({ user, tokoId, initialData = [] }) {
  ...
  const brandIndex = useMemo(() => {
    ...
    if (!Array.isArray(idx) || idx.length === 0) {
      console.warn("[InputPenjualan] getBrandIndex() kosong — cek MasterDataHargaPenjualan.js");
    }
    return idx;
  }, []);

  const baseBrandOptions = useMemo(() => unique(brandIndex.map((b) => b.brand)), [brandIndex]);
  ...
  const salesOptions = useMemo(() => {
    ...
    if (!arr.length) console.warn("[InputPenjualan] getSalesByToko() kosong — cek ListDataPenjualan.js");
    return arr;
  }, [tokoName]);
  ...
  const [form, setForm] = useState(() => ({
    ...
    // identitas
    tokoRef: TOKO_LIST?.includes(tokoName) ? tokoName : TOKO_LIST?.[0] || tokoName,
    salesName: "",
    shName: "",
    slName: "",
    tuyulName: "",
  ...
  useEffect(() => {
    if (_productOptions.length && !form.produk) {
      setForm((f) => ({ ...f, produk: _productOptions[0], namaBarang: f.namaBarang || _productOptions[0] }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const bateraiOptions = useMemo(() => unique(getBateraiByBrandProduct(form.brand, form.produk) || []), [form.brand, form.produk]);
  const chargerOptions = useMemo(() => unique(getChargerByBrandProduct(form.brand, form.produk) || []), [form.brand, form.produk]);

  // Auto pilih default baterai/charger pertama saat ada
  useEffect(() => {
    if (bateraiOptions.length && !form.baterai) setForm((f) => ({ ...f, baterai: bateraiOptions[0] }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bateraiOptions.length]);

  useEffect(() => {
    if (chargerOptions.length && !form.charger) setForm((f) => ({ ...f, charger: chargerOptions[0] }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    const sales =
      salesOptions.find((s) => (s.name || "").toLowerCase() === form.salesName.toLowerCase()) || null;
    if (sales) {
      setForm((f) => ({
        ...f,
        nik: f.nik || sales.nik || "",
  ...
  useEffect(() => {
    if (form.namaBrand && !form.brand) setForm((f) => ({ ...f, brand: f.namaBrand }));
  }, [form.namaBrand, form.brand]);
  useEffect(() => {
    if (form.namaBarang && !form.produk) setForm((f) => ({ ...f, produk: f.namaBarang }));
  ...
  const handleImportExcel = async (e) => {
    ...
    try {
      ...
      const sheetName = wb.SheetNames.find((n) => /input|penjualan|list|po/i.test(n)) || wb.SheetNames[0];
      ...
      alert("File Excel tidak dikenali. Pastikan header kolom sesuai template.");
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Input Penjualan — {tokoName}</h1>
          <p className="text-slate-600">
            Semua dropdown diambil dari <code>ListDataPenjualan.js</code> &{" "}
            <code>MasterDataHargaPenjualan.js</code>. Klik Brand → Produk/Warna/Baterai/Charger akan muncul.
          </p>
        </div>
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <div className="md:col-span-2">
            <label className="text-xs text-slate-600">Pilih SL & Freelance</label>
            <div className="grid grid-cols-2 gap-2">
              <select
                className="border rounded px-2 py-1"
                value={form.slName}
          ...
          <div className="md:col-span-2">
            ...
            {imeiOptions.length > 0 ? (
              <select
                ...
                value={form.noImei}
                onChange={(e) =>
                  setForm({ ...form, noImei: e.target.value, imei1: e.target.value })
                }
              >
                <option value="">— Pilih —</option>
                {imeiOptions.map((x) => (
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <div>
            <label className="text-xs text-slate-600">Pembayaran Melalui (Leasing)</label>
            <input
              className="w-full border rounded px-2 py-1"
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <div className="md:col-span-2">
            <label className="text-xs text-slate-600">Accessories / Sparepart</label>
            <input
              className="w-full border rounded px-2 py-1"
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <tbody>
              {pageRows.map((row) => {
                ...
                if (isEditing && editDraft) {
                  const fe = computeFinancials(editDraft, row.tokoRef || tokoName);
                  return (
                    <tr key={row.id} className="border-b last:border-0 bg-slate-50/50">
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          onChange={(e) =>
                            setEditDraft((d) => ({ ...d, namaBrand: e.target.value }))
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          onChange={(e) =>
                            setEditDraft((d) => ({ ...d, namaBarang: e.target.value }))
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          onChange={(e) =>
                            setEditDraft((d) => ({ ...d, noImei: e.target.value }))
                          }
                        />
                      </td>
                      ...
                      <td className="px-3 py-2 text-right">{fmtIDR(fe.subtotal)}</td>
                      ...
                      <td className="px-3 py-2 text-right">{fmtIDR(fe.cicilan)}</td>
                      <td className="px-3 py-2 text-right">{fmtIDR(fe.grandTotal)}</td>
                      <td className="px-3 py-2">
                ...
                return (
                  <tr key={row.id} className="border-b last:border-0">
                    ...
                    <td className="px-3 py-2">{row.noImei || row.imei1 || "-"}</td>
                    ...
                    <td className="px-3 py-2 text-right">{Number(f.mdrPct).toFixed(2)}</td>
                    ...
                    <td className="px-3 py-2 text-right">{fmtIDR(row.dpMerchant)}</td>
                    <td className="px-3 py-2 text-right">{fmtIDR(row.dpToko)}</td>
                    <td className="px-3 py-2 text-right">{fmtIDR(row.dpTalangan)}</td>
        ...
        <div className="mt-4 flex flex-col md:flex-row items-center justify-between gap-2">
          <div className="text-sm text-slate-600">
            Menampilkan {filteredRows.length === 0 ? 0 : startIdx + 1}–{Math.min(endIdx, filteredRows.length)} dari {filteredRows.length} baris
          </div>
          <div className="flex items-center gap-2">
            <button
      ...
      <p className="text-xs text-slate-500">
        * Jika dropdown masih kosong, cek kembali file sumber di folder <code>data</code> (brandIndex,
        getSalesByToko, baterai/charger, dll). Saya sudah menambahkan <code>console.warn</code> untuk memandu.
      </p>
    </div>
  );
}
